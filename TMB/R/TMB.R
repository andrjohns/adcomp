...
##' \itemize{
##'   \item \code{tol} Drop elements with \code{abs(L)<tol} recursively while calculating new rows of \code{L}.
...
##'   \item \code{perm} Specifies the elimination order; Natural order by default.
##'   \item \code{adaptive} TRUE by default; Redo symbolic analysis gradually if the incomplete factor becomes inaccurate (\code{error} > \code{tol} * \code{relax}).
##'   \item \code{parallel} FALSE by default; Run incomplete factorization and its adjoint code using OpenMP. This option normally requires an elimination order (\code{perm}) to provide speedups.
##' }
